# 🧠 Os Studio — Brain-GitHub 동기화 아키텍처 레퍼런스
...
| 설정 키 | 용도 | 기본값 |
|---------|------|--------|
| `osStudio.localBrainPath` | 로컬 지식 폴더 절대 경로 | `""` (비면 `~/.os-studio-brain`) |
| `osStudio.secondBrainRepo` | 깃허브 저장소 URL | `""` |
| `osStudio.ollamaUrl` | AI 서버 주소 | `http://127.0.0.1:11434` |
| `osStudio.defaultModel` | AI 모델 이름 | `gemma4:e2b` |
| `osStudio.requestTimeout` | AI 응답 대기 시간(초) | `300` |
...
_getBrainDir()               // 로컬 지식 폴더 경로 반환 (미설정 시 ~/.os-studio-brain)
...
# VS Code Settings → osStudio.localBrainPath 값 확인

# 2. 깃허브 URL이 저장되어 있는지?
# VS Code Settings → osStudio.secondBrainRepo 값 확인
...
# → os-studio-{version}.vsix

# GitHub 릴리즈
gh release create v{version} os-studio-{version}.vsix -t "Release v{version}"
